-- A modification of the original nn.Jacobian, in order to test
-- the backpropagation of graph-related layers
...
function nn.Jacobian.backward(module, input, param, dparam)
   ...
   local dout = module.output[2].new():resizeAs(module.output[2])
   -- 1D view
   local sdout = module.output[2].new(dout:storage(),1,dout:nElement())
...
function nn.Jacobian.forward(module, input, param, perturbation)
   ...
   -- jacobian matrix to calculate
   local jacobian = torch.Tensor():resize(param:nElement(),module:forward(input)[2]:nElement())
   ...
   for i=1,sin:nElement() do
      ...
      outa:copy(module:forward(input)[2])
      sin[i] = orig + perturbation
      outb:copy(module:forward(input)[2])
      sin[i] = orig
...
function nn.Jacobian.backwardDiagHessian(module, input, diagHessianParamName)
   ...
   module:forward(input)
   local diagHessianOutput = module.output[2].new():resizeAs(module.output[2]):fill(1)

   module.diagHessianWeight:zero()
   module.diagHessianBias:zero()
   module:updateDiagHessianInput(input[2], diagHessianOutput)
   module:accDiagHessianParameters(input[2], diagHessianOutput)
...
function nn.Jacobian.linearModuleDiagHessian(module, input, gradParamName)
   ...
   module:forward(input)
   local gradOutput = module.output[2].new():resizeAs(module.output[2])
...
function nn.Jacobian.testJacobian(module, input, minval, maxval, perturbation)
   ...
   input[2]:copy(torch.rand(input[2]:nElement()):mul(inrange):add(minval))
...
function nn.Jacobian.testIO(module,input, minval, maxval)
   ...
   -- run module
   module:forward(input)
   local go = module.output[2]:clone():copy(torch.rand(module.output[2]:nElement()):mul(inrange):add(minval))
   module:zeroGradParameters()
   module:updateGradInput(input,go)
   module:accGradParameters(input,go)

   local fo = module.output[2]:clone()
...
function nn.Jacobian.testAllUpdate(module, input, weight, gradWeight)
   ...
   gradOutput = torch.rand(maccgp.output[2]:size())
