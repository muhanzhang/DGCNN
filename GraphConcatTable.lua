local GraphConcatTable, parent = torch.class('nn.GraphConcatTable', 'nn.Container')

function GraphConcatTable:__init()
   parent.__init(self)
   self.modules = {}
   self.output = {}
end

function GraphConcatTable:updateOutput(input)
...
local function backward(self, method, input, gradOutput, scale)
   ...
   self.gradInput = (not wasTable) and self.gradInput or input[2]:clone()
...
function GraphConcatTable:updateGradInput(input, gradOutput)
   return backward(self, 'updateGradInput', input, gradOutput)
end

function GraphConcatTable:backward(input, gradOutput, scale)
   return backward(self, 'backward', input, gradOutput, scale)
end

function GraphConcatTable:accGradParameters(input, gradOutput, scale)
   scale = scale or 1
   for i,module in ipairs(self.modules) do
...
function GraphConcatTable:accUpdateGradParameters(input, gradOutput, lr)
   for i,module in ipairs(self.modules) do
      self:rethrowErrors(module, i, 'accUpdateGradParameters', input, gradOutput[i], lr)
   end
end

function GraphConcatTable:__tostring__()
   local tab = '  '
   local line = '\n'
   local next = '  |`-> '
   local lastNext = '   `-> '
